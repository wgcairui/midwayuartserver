...
/* private getProtocolAlias(mac: string, pid: string | number, protocol: string, name: string) {
        const alias = ctx.Cache.CacheAlias.get(mac + pid + protocol)
...
export async function terminalDataCheck(
  ...
  if (setup.AlarmStat.size > 0) {
    checkAlarm(result, setup.AlarmStat).forEach(el => {
      const value = RedisService.parseUnit(el.unit!, el.value);
      if (value) {
        el.alarm = true;
        AlarmEvents.push({
          argument: el.name,
 ...
 * @param Query
 */
export async function checkUPS(Query: Uart.queryResult): Promise<alarm | void> {
