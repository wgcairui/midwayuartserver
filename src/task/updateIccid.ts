...
export class UpdateIccid {
    ...
    async up() {
        ...
        const terminalsFilter = terminals.filter(el => el.ICCID && el.ICCID.length === 20)
        ...
        for (const ter of terminalsFilter) {
            ...
            if (info.code === 'OK') {
                ...
                const iccidInfo: Partial<Uart.iccidInfo> = data ? { statu: true, ...data } : { statu: false }
                await this.Device.setTerminal(ter.DevMac, { iccidInfo: iccidInfo as any })
