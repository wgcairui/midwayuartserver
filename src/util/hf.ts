...
export class HF {
  ...
  async macInfo(mac = '') {
    const url =
      'http://open.bridge.iotworkshop.com:8080/iotbs/api/v1/devices/?size=10&page=0';
    const data = {
      hostName: '',
      mac,
      moduleType: '',
  ...
  async getUserId() {
    if (!this.uerId) {
      const url =
        'http://open.bridge.iotworkshop.com:8080/iotbs/api/v1/users/info?null';
  ...
  async macRemote(mac: string) {
    ...
    return macInfo.result !== 0
      ? ''
      : `http://bridge.iotworkshop.com/ctrl/device/index.html?remote_code=${macInfo.data.content[0].remoteCode}`;
  }
  ...
  private async post<T>(url: string, data?: any) {
    ...
    return axios
      .post<T>(url, data, { headers: { 'Access-Token': this.token } })
      .then(el => el.data);
  }
  ...
  private async get<T>(url: string) {
    ...
    return axios
      .get<T>(url, { headers: { 'Access-Token': this.token } })
      .then(el => el.data);
  }
}
