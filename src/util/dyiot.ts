...
export class DyIot {
  ...
  @Init()
  async init() {
    const key = await getModelForClass(SecretApp).findOne({ type: 'dyIot' }).lean()
  ...
  async QueryCardFlowInfo(iccid: string) {
    let req = new $Dyiotapi.QueryCardFlowInfoRequest({
      iccid
    });
  ...
  async DoIotUnbindResume(iccid: string) {
    let req = new $Dyiotapi.DoIotUnbindResumeRequest({
      iccid
    });
  ...
  async DoIotPostImei(imei: string, comments: string, deviceType: string) {
    let req = new $Dyiotapi.DoIotPostImeiRequest({
      imei,
      comments,
      deviceType
    });
   ...
   */
  async DoIotChgBindOrUnBindRequest(iccid: string, imei: string, newImei: string, opionType: "unBind" | "chgBind") {
    let doIotChgBindOrUnBindRequest = new $Dyiotapi.DoIotChgBindOrUnBindRequest({
      iccid,
      imei,
      newImei,
      opionType,
    ...
    const res = await this.iot.doIotChgBindOrUnBind(doIotChgBindOrUnBindRequest);
    return res.body
  }
  ...
  async QueryCardInfo(iccid: string) {
    let queryCardInfoRequest = new $Dyiotapi.QueryCardInfoRequest({
      iccid,
    });
    // 复制代码运行请自行打印 API 的返回值
  ...
  async QueryIotCardOfferDtl(iccid: string) {
    let queryIotCardOfferDtlRequest = new $Dyiotapi.QueryIotCardOfferDtlRequest({
      iccid,
    });
    // 复制代码运行请自行打印 API 的返回值
    const res = await this.iot.queryIotCardOfferDtl(queryIotCardOfferDtlRequest)
    return res.body
  }
   ...
   */
  async DoIotRecharge(iccid: string, offerIds: string = Date.now().toString(), outId: string = Date.now().toString(), effCode: "1000" | "AUTO_ORD" = "1000") {
    let doIotRechargeRequest = new $Dyiotapi.DoIotRechargeRequest({
      iccid,
      offerIds,
      outId,
      effCode,
  ...
  parseIotDate(date: string) {
    const str = `${date.slice(0, 4)}-${date.slice(4, 6)}-${date.slice(6, 8)} ${date.slice(8, 10)}:${date.slice(10, 12)}`
