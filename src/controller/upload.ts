...
export class Oss {
   ...
   */
    ...
    async uploads() {
        ...
        return {
            code: 200,
            data: await this.ossService.put(`${file.type.split("/").reverse()[0]}/${MD5(file.name).toString()}.${fileType}`, file.path)
        };
    }
    ...
    async ossFilelist(@Body() prefix?: string) {
        ...
        const data = Object.prototype.hasOwnProperty.call(files, "objects") && files.objects ?
            files.objects.map(({ url, lastModified, size, name }) => ({ url, lastModified, size, name }))
            : []
        return {
            code: 200,
            data
